            ...
            for up in res.get("result", []):
                ...
                if txt == "/start":
                    if not n:
                        ...
                        send_msg(cid, "🌲 Ласкаво просимо до Клану! Твій шлях починається тут.")
                    else: send_msg(cid, "🐾 Ти вже в строю!")
                    ...
                    if "мій новак" in txt:
                        ...
                        nn = m["text"][6:].strip()
                        if nn:
                            conn = sqlite3.connect(DB_NAME); conn.execute("UPDATE novaky SET name=? WHERE user_id=?", (nn, uid))
                            conn.commit(); conn.close(); send_msg(cid, f"✨ Відтепер тебе знають як {nn}!")
                        ...
                        send_msg(cid, "💥 Твій новак покинув Клан. Весь прогрес стерто.")
                    ...
                    # Квести
                        ...
                        if not row or row[0] != get_today_str(): generate_daily_quests(uid); send_msg(cid, "📜 Оновлюю список..."); continue
                        
                        def format_q(qid, val, done):
                            inf = QUEST_TEMPLATES[qid]
                            d = inf['desc'].format(val) if val > 0 else inf['desc']
                            return f"{d} {'✅' if done else '❌'} (⚡-{inf['energy']})"
                        
                        send_msg(cid, f"📜 **Справи на сьогодні:**\n1. {format_q(row[1], row[2], row[3])}\n2. {format_q(row[4], row[5], row[6])}")

                    elif txt in ["завдання 1", "завдання 2"]:
                        if n["is_sleeping"]: send_msg(cid, "💤 Новак спить!"); continue
                        now = int(time.time())
                        if now - n["last_quest_time"] < 3600: 
                            send_msg(cid, f"⏳ Клан вимагає відпочинку! Зачекай ще {(3600-(now-n['last_quest_time']))//60} хв."); continue
                        
                        conn = sqlite3.connect(DB_NAME); cursor = conn.cursor()
                        cursor.execute("SELECT q1_id, q1_val, q1_done, q2_id, q2_val, q2_done FROM user_quests WHERE user_id=?", (uid,))
                        q_data = cursor.fetchone()
                        idx = 0 if "1" in txt else 3
                        if q_data[idx+2]: send_msg(cid, "✅ Вже виконано!"); conn.close(); continue
                        
                        inf = QUEST_TEMPLATES[q_data[idx]]
                        if n['energy'] < inf['energy']: send_msg(cid, "🪫 Мало сил!"); conn.close(); continue
                        
                        col = "q1_done" if "1" in txt else "q2_done"
                        ...
                        conn.commit(); conn.close(); send_msg(cid, f"🌟 Виконано! +{inf['exp']} досвіду.")
                    ...
                    # Соціальне
                        ...
                        if txt == "подружитися":
                            ...
                            send_msg(cid, f"😾 Бійка! {n['name']} покусав {tn['name']}! (-20, разом: {p})")
                        elif txt == "пара":
                            p1, p2 = min(uid, tid), max(uid, tid)
                            conn = sqlite3.connect(DB_NAME); row = conn.execute("SELECT points FROM relationships WHERE user_one=? AND user_two=?", (p1, p2)).fetchone()
                            if row and row[0] >= 60: update_relation(uid, tid, 10, "Пара"); send_msg(cid, "❤️ Тепер ви пара!")
                            else: send_msg(cid, f"❌ Потрібно 60 балів (у вас {row[0] if row else 0}).")
                    ...
                    # Дії
                        ...
                        conn = sqlite3.connect(DB_NAME); conn.execute("UPDATE novaky SET meat=meat+?, energy=energy-25, exp=exp+? WHERE user_id=?", (m_g, e_g, uid))
                        conn.commit(); conn.close(); send_msg(cid, f"🏹 Вдале полювання! +{m_g} кг м'яса, +{e_g} досвіду.")

                    elif "їсти" in txt:
                        if n['meat'] > 0:
                            conn = sqlite3.connect(DB_NAME); conn.execute("UPDATE novaky SET meat=meat-1, energy=min(100, energy+20) WHERE user_id=?", (uid,))
                            conn.commit(); conn.close(); send_msg(cid, "🍴 М'ясо додало сил! +20 ⚡")
                        else: send_msg(cid, "🥩 Рюкзак порожній. Час на полювання!")

                    elif "спати" in txt:
                        conn = sqlite3.connect(DB_NAME); conn.execute("UPDATE novaky SET is_sleeping=1, last_rest=? WHERE user_id=?", (int(time.time()), uid))
                        conn.commit(); conn.close(); send_msg(cid, "💤 На добраніч.")

                    elif "прокинутись" in txt:
                        conn = sqlite3.connect(DB_NAME); conn.execute("UPDATE novaky SET is_sleeping=0 WHERE user_id=?", (uid,))
                        conn.commit(); conn.close(); send_msg(cid, "☀️ Доброго ранку!")
