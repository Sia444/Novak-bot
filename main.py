...
def run():
    # Render автоматично дає порт у змінну оточення PORT. Якщо її немає — беремо 8080
...
def update_relation(u1, u2, points_add, set_status=None):
    ...
    if set_status: 
        status = set_status
    elif new_points >= 50 and status == "Знайомі": 
        status = "Друзі"
    elif new_points <= -30 and status == "Знайомі": 
        status = "Вороги"
    elif new_points > -30 and new_points < 50 and status in ["Друзі", "Вороги"]: 
        status = "Знайомі"
    conn = sqlite3.connect(DB_NAME)
            ...
            for update in res.get("result", []):
                ...
                if text == "/start":
                    if not n:
                        ...
                        now = int(time.time())
                        if n["is_sleeping"]: 
                            send_msg(cid, "💤 Розбуди його спочатку!")
                        elif n["energy"] < 20: 
                            send_msg(cid, "🪫 Мало енергії для полювання (треба хоча б 20 ⚡).")
                        else:
                            ...
                            conn.commit()
                            conn.close()
                            send_msg(cid, f"🏹 Полювання у **{loc['n']}**! Твій новак здобув +{loc['m']} кг м'яса та отримав +{loc['e']} ✨ досвіду.")
                        ...
                        if text == "подружитися":
                            ...
                            if current_rel["points"] < 60:
                                ...
                                conn.commit()
                                conn.close()
                                send_msg(cid, f"❤️ ✨ **{n['name']}** та **{target_n['name']}** тепер офіційно стали paru! ✨ ❤️")
                        ...
                        if n["meat"] > 0:
                            ...
                            send_msg(cid, "🥩 Немає м'яса! Спочатку сходи на полювання або виконай квест.")
                        
                    elif "тренувати" in text:
                        if n["is_sleeping"]: 
                            send_msg(cid, "💤 Новак спить! Розбуди його спочатку.")
                        elif n["energy"] < 30: 
                            send_msg(cid, "🪫 Мало енергії для тренування (треба хоча б 30 ⚡).")
                        else:
                            exp_gain = random.randint(20, 40)
                            conn = sqlite3.connect(DB_NAME)
                            conn.execute("UPDATE novaky SET exp=exp+?, energy=energy-30, last_train=? WHERE user_id=?", (exp_gain, int(time.time()), uid))
                            conn.commit()
                            conn.close()
                            send_msg(cid, f"⚔️ Тренування пройшло успішно! Досвід: +{exp_gain} ✨. (-30 🔋)")
