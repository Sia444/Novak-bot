...
def get_novak(uid):
    ...
    now = int(time.time())
    name, meat, exp, energy, l_rest, slp, ph, l_qt = row
    rec = (now - l_rest) // (100 if slp else 300)
    if rec > 0:
        energy = min(100, energy + rec)
        conn.execute("UPDATE novaky SET energy=?, last_rest=? WHERE user_id=?", (energy, now, uid))
        conn.commit()
    conn.close()
        ...
        try:
            ...
            for up in res.get("result", []):
                ...
                if txt == "/start":
                    ...
                    if "мій новак" in txt:
                        ...
                        if os.path.exists(photo):
                            with open(photo, 'rb') as f:
                                requests.post(URL + "sendPhoto", data={'chat_id': cid, 'caption': cap, 'parse_mode': 'Markdown'}, files={'photo': f})
                        ...
                        if nn:
                            conn = sqlite3.connect(DB_NAME)
                            conn.execute("UPDATE novaky SET name=? WHERE user_id=?", (nn, uid))
                            conn.commit(); conn.close()
                            send_msg(cid, f"✨ Тепер тебе звуть {nn}")

                    # СТОСУНКИ З КД 1 ГОДИНА
                        ...
                        conn = sqlite3.connect(DB_NAME)
                        cooldown_row = conn.execute("SELECT last_interact FROM rel_cooldowns WHERE u1=? AND u2=?", (p1, p2)).fetchone()
                        now = int(time.time())
                        
                        if cooldown_row and (now - cooldown_row[0] < 3600):
                            remain = (3600 - (now - cooldown_row[0])) // 60
                            send_msg(cid, f"⏳ Ваші коти ще виснажені від останньої зустрічі. Зачекайте ще {remain} хв."); conn.close(); continue
                        ...
                        else: send_msg(cid, f"❌ Мало прихильності ({row[0] if row else 0}/60).")
                        conn.close()
                    ...
                    # ЗАВДАННЯ, ПОЛЮВАННЯ, СОН (Залишаються без змін)
                    elif txt == "завдання":
                        conn = sqlite3.connect(DB_NAME); cursor = conn.cursor()
                        ...
                        def fmt(qid, val, done):
                            info = QUEST_TEMPLATES[qid]; d = info['desc'].format(val) if val > 0 else info['desc']
                            return f"{d} {'✅' if done else '❌'} (⚡-{info['energy']})"
                        ...
                        now = int(time.time())
                        if now - n["last_quest_time"] < 3600: send_msg(cid, f"⏳ Почекай ще {(3600-(now-n['last_quest_time']))//60} хв."); continue
                        conn = sqlite3.connect(DB_NAME); cursor = conn.cursor()
                        ...
                        if q_data[idx+1]: send_msg(cid, "✅ Готово!"); conn.close(); continue
                        inf = QUEST_TEMPLATES[q_data[idx]]
                        if n['energy'] < inf['energy']: send_msg(cid, f"🪫 Треба {inf['energy']}⚡"); conn.close(); continue
                        ...
                        conn.commit(); conn.close(); send_msg(cid, f"🏹 +{m_g} кг м'яса, +{e_g} досвіду.")

                    elif "їсти" in txt:
                        if n['meat'] > 0:
                            conn = sqlite3.connect(DB_NAME); conn.execute("UPDATE novaky SET meat=meat-1, energy=min(100, energy+20) WHERE user_id=?", (uid,))
                            conn.commit(); conn.close(); send_msg(cid, "🍴 +20 ⚡")
                        else: send_msg(cid, "🥩 Порожньо!")

                    elif "спати" in txt:
                        conn = sqlite3.connect(DB_NAME); conn.execute("UPDATE novaky SET is_sleeping=1, last_rest=? WHERE user_id=?", (int(time.time()), uid)); conn.commit(); conn.close(); send_msg(cid, "💤 Спиш.")

                    elif "прокинутись" in txt:
                        conn = sqlite3.connect(DB_NAME); conn.execute("UPDATE novaky SET is_sleeping=0 WHERE user_id=?", (uid,)); conn.commit(); conn.close(); send_msg(cid, "☀️ Встав.")

        except Exception as e: print(f"Помилка: {e}"); time.sleep(2)
