                ...
                if txt == "/start":
                    ...
                    # Квести
                        ...
                        if not row or row[0] != get_today_str():
                            generate_daily_quests(uid)
                            send_msg(cid, "📜 Клан готує нові завдання... Напиши 'завдання' ще раз.")
                        else:
                            def fmt(qid, val, done):
                                info = QUEST_TEMPLATES[qid]
                                d = info['desc'].format(val) if val > 0 else info['desc']
                        ...
                        if now - n["last_quest_time"] < 3600:
                            send_msg(cid, f"⏳ Зачекай {(3600-(now-n['last_quest_time']))//60} хв."); continue
                        ...
                        send_msg(cid, f"🌟 Виконано! -{inf['energy']}⚡, +{inf['exp']} досвіду.")
                    ...
                    # Стосунки
                        ...
                        if txt == "подружитися":
                            ...
                            else: send_msg(cid, f"❌ Мало прихильності (у вас {row[0] if row else 0}/60).")
